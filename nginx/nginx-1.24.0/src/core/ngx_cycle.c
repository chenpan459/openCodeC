...
{
    ngx_uint_t        i;              // 循环计数器
    ngx_shm_zone_t   *shm_zone;      // 指向共享内存区域的指针
    ngx_list_part_t  *part;          // 指向共享内存列表部分的指针
    ...
    for (i = 0; /* void */ ; i++) {
        ...
        // 如果当前部分已经遍历完，检查是否有下一个部分
        if (i >= part->nelts) {
            if (part->next == NULL) {
                break;  // 没有下一个部分，退出循环
            }
            part = part->next;  // 移动到下一个部分
            shm_zone = part->elts;
            i = 0;  // 重置计数器
        }
        ...
        if (size && size != shm_zone[i].shm.size) {
            ...
            return NULL;  // 大小冲突，返回NULL
        }

        return &shm_zone[i];  // 找到匹配的共享内存区域，返回其指针
    }

    // 如果没有找到匹配的共享内存区域，则添加新的共享内存区域
    shm_zone = ngx_list_push(&cf->cycle->shared_memory);

    if (shm_zone == NULL) {
        return NULL;  // 添加失败，返回NULL
    }
    ...
    return shm_zone;  // 返回新添加的共享内存区域的指针
}
