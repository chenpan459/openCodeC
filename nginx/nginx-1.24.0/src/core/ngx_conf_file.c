...
ngx_conf_parse(ngx_conf_t *cf, ngx_str_t *filename)
{
    ...
    for ( ;; ) {
       ...
       // printf("conf_file buffer=%s\n", cf->conf_file->buffer->start); 
       //  printf("conf_file buffer=%s\n", cf->conf_file->dump->start);       
     //   cf-> 
        ...
        if (cf->handler) {
          ...
          //  printf("conf_file cf->handler_conf=%s\n", (char*)cf->handler_conf);      
            if (rv == NGX_CONF_OK) {
                continue;
            }
...
ngx_conf_read_token(ngx_conf_t *cf)
{
    ...
    //printf("cf->name:%s file_size=%ld\n", cf->conf_file->file.name.data,file_size);
    ...
    for ( ;; ) {
        ...
        if (last_space) {
            ...
            case '{':
                ...
                if (ch == '{') {
                     //printf("[%s-%d]#########NGX_CONF_BLOCK_START#########cf->args->nelts=%ld\n",__func__,__LINE__,cf->args->nelts);
                    return NGX_CONF_BLOCK_START;
                }
            ...
            case '}':
               ...
               //printf("[%s-%d]#########NGX_CONF_BLOCK_DONE#########cf->args->nelts=%ld\n",__func__,__LINE__,cf->args->nelts);
            ...
            if (found) {
                ...
                *dst = '\0';
                word->len = len;
                //printf("word->len = %ld word->data=%s\n",word->len ,word->data);
