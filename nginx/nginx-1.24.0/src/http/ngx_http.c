...
{
    char                        *rv; // 返回值
    ngx_uint_t                   mi, m, s; // 模块索引、模块循环变量、服务器循环变量
    ngx_conf_t                   pcf; // 保存原始配置结构
    ngx_http_module_t           *module; // HTTP模块指针
    ngx_http_conf_ctx_t         *ctx; // HTTP配置上下文
    ngx_http_core_loc_conf_t    *clcf; // 核心位置配置
    ngx_http_core_srv_conf_t   **cscfp; // 服务器配置指针数组
    ngx_http_core_main_conf_t   *cmcf; // 核心主配置
    // 检查是否已经存在配置上下文
