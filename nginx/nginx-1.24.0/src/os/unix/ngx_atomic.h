...
/*是 GCC 提供的一组内置原子操作函数之一，用于实现原子的比较并交换操作。以下是关于该函数的详细说明：

1. 函数原型
c
bool __sync_bool_compare_and_swap (type *ptr, type oldval, type newval, ...)
2. 功能
原子比较并交换：该函数会将指针 ptr 所指向的值与 oldval 进行比较。
如果 *ptr == oldval，则将 *ptr 的值更新为 newval，并返回 true。
如果 *ptr != oldval，则不进行任何修改，直接返回 false
*/
#define ngx_atomic_cmp_set(lock, old, set)                                       \
    __sync_bool_compare_and_swap(lock, old, set)
 
 
 //  定义一个宏 ngx_atomic_fetch_add，用于原子加法操作 
 //参数 value: 指向要操作的原子变量的指针 参数 add: 要加的值 返回值: 返回原子变量加法操作前的值
#define ngx_atomic_fetch_add(value, add)                                      \
    __sync_fetch_and_add(value, add)

/*内存屏障：__sync_synchronize 创建一个完整的内存屏障（full memory barrier），确保在其之前的所有读写操作都完成之后，才开始执行其后的所有读写操作。
顺序一致性：它保证了在多核处理器上，不同线程之间的内存操作顺序的一致性。*/
